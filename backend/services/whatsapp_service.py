...
async def send_voice_message(text: str, chat_id: Optional[str] = None, context_type: str = "text") -> bool:
    ...
    try:
        ...
        return False   # do NOT silently fall back to text

    # Step 2: send via /send-voice
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code != 200:
                body = resp.text[:300]
                logger.error(f"[TTS] FAILED: /send-voice returned {resp.status_code}: {body}")
                return False
            logger.info(f"[TTS] Voice note sent OK ({resp.status_code})")
            return True
    except Exception as e:
        logger.error(f"[TTS] FAILED at /send-voice request: {type(e).__name__}: {e}")
        logger.error(f"[TTS] Traceback:\n{traceback.format_exc()}")
        return False   # do NOT silently fall back to text
